...
| Branch | `main` contains the promoted delivery history through `69273cb`; retained branch `codex/sunnah-everyday-build` remains at that promoted commit |
...
| GitHub push / PR | On explicit owner direction, `main` was fast-forwarded from `137521e` to `69273cb` and pushed successfully; the feature branch remains at `69273cb`. No PR was used. `gh auth status` has no logged-in host, so protected repository settings cannot be inspected from this environment |
